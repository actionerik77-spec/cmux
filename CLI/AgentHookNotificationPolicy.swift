...
enum AgentHookNotificationPolicy {
    ...
    static func redactSensitiveCommand(_ value: String) -> String {
        ...
        let patterns: [(pattern: String, replacement: String)] = [
            ...
            (#"(?i)\b[A-Za-z_]*(?:api[_-]?key|password|passwd|secret|token|authorization|cookie|passphrase|pass)[A-Za-z0-9_]*\s*=\s*(?:'[^']*'|\"[^\"]*\"|[^\s;&|]+)"#, "<credential>=<token>"),
            (#"(?i)\b(?:api[_-]?key|password|passwd|secret|token|authorization|cookie|passphrase|pass)\s*=\s*[^\s;&|]+"#, "<credential>=<token>"),
