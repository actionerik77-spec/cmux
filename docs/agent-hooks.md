...
The Claude wrapper does not install a catch-all `PreToolUse` hook. Ordinary tool calls stay inside Claude Code and do not launch a cmux hook process or append Feed telemetry. Targeted hooks remain for lifecycle changes, permission requests, `AskUserQuestion`, `ExitPlanMode`, `CronCreate`, and `PushNotification`. Blocking questions and plan approvals are correlated by Claude's tool ID and bounded canonical payload: resolved PermissionRequest decisions retire denied calls even when Claude omits `tool_use_id`, while oversized or ambiguous payloads fail closed until an explicit completion ID arrives. If the Feed bridge times out, Claude's native permission prompt remains authoritative and cmux clears only its app-owned lifecycle state; no ordinary-tool hook is installed. Claude's delayed permission Notification contains no request identity, so cmux represents concurrent native permission notices as one session aggregate and clears it only after the last blocker resolves. Bypass-permissions prompts use an ephemeral request-scoped attention signal that writes no Feed card, event record, or durable notification history. Resolving one request never clears another request's pane attention. Ephemeral attention is retired at explicit turn or tool completion, exact process-generation exit, workspace close, or bounded-registry eviction; local releases are authenticated to the recorded process generation, and recoverable persistent remote PTY disconnects retain attention until completion or workspace close. The in-memory request registry is capped at 256 entries and uses no timer. Session mapping files are rewritten only when their stored state changes.

A PermissionRequest still invokes the targeted Feed decision bridge. That decision may append one event to `~/.cmuxterm/workstream.jsonl` and cmux's bounded event log, and normal search indexing may update its SQLite database or WAL for that event. Ordinary tool calls no longer create that per-call write load. If endpoint-security policy requires no wrapper-injected Claude hooks, turn off **Settings > Automation > Claude Code Integration** or set:
...
This disables cmux's Claude session tracking, status updates, Feed permission bridge, notifications, and automatic session resume. The setting is captured when a terminal surface starts, so open a new terminal surface and restart Claude after changing it. For one launch without changing Settings, use `CMUX_CLAUDE_HOOKS_DISABLED=1 claude`.
...
| --- | --- | --- |
| Claude Code | `CLAUDE_CONFIG_DIR` | `CMUX_CLAUDE_HOOKS_DISABLED=1` |
